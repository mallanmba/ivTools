 ...
 *
 *  Changes:
 *  - ivview ported from Motif to SoWin/SoQt
...
//#   include <Inventor/Xt/viewers/SoXtWalkViewer.h>  // Coin port
#  else
#   include <Inventor/Qt/SoQt.h>
#   include <Inventor/Qt/viewers/SoQtExaminerViewer.h>
//#   include <Inventor/Xt/viewers/SoXtWalkViewer.h>  // Coin port
//#   include "ivviewMenus.h"  // Coin port
#  endif
#endif
//#include "../../samples/widgets/MyColorEditor.h" // Coin port by PCJohn
...
typedef struct {
    ...
    SoGroup *super;      // hidden top of the scene, added by PCJohn as workaround of overlays
...
#ifdef USE_WIN32 
static void initPerfMeter(SoWinViewer *vwr);
static void viewStartCB(void *, SoWinViewer *);
#else
static void initPerfMeter(SoQtViewer *vwr);
static void viewStartCB(void *, SoQtViewer *);
#endif

// Global variables
#ifdef USE_WIN32
static SoWinViewer *viewer = NULL;
#else
static SoQtViewer *viewer = NULL;
#endif
//static Widget shell = NULL;
static SoOrthographicCamera *overlayCamera;  // overlay workaround by PCJohn
static SbBool useWalkViewer = FALSE;
static FileInfo files = { 0, NULL, NULL, NULL, NULL, NULL };
...
static char hourglass_bits[] = {
   // char retyping done by PCJohn because of MSVC 6 warnings
   (char)0x00, (char)0x00, (char)0x00, (char)0xfe, (char)0xff, (char)0x00,
   (char)0x04, (char)0x40, (char)0x00, (char)0x04, (char)0x40, (char)0x00,
   (char)0x04, (char)0x40, (char)0x00, (char)0xe8, (char)0x2e, (char)0x00,
   (char)0xd0, (char)0x17, (char)0x00, (char)0xa0, (char)0x0b, (char)0x00,
   (char)0x40, (char)0x05, (char)0x00, (char)0x40, (char)0x05, (char)0x00,
   (char)0x40, (char)0x04, (char)0x00, (char)0x40, (char)0x04, (char)0x00,
   (char)0x40, (char)0x04, (char)0x00, (char)0x20, (char)0x09, (char)0x00,
   (char)0x10, (char)0x11, (char)0x00, (char)0x88, (char)0x23, (char)0x00,
   (char)0xc4, (char)0x47, (char)0x00, (char)0xe4, (char)0x4f, (char)0x00,
   (char)0xf4, (char)0x5f, (char)0x00, (char)0xfe, (char)0xff, (char)0x00,
   (char)0x00, (char)0x00, (char)0x00};

static char hourglass_mask_bits[] = {
   // char retyping done by PCJohn because of MSVC 6 warnings
   (char)0xff, (char)0xff, (char)0x01, (char)0xff, (char)0xff, (char)0x01,
   (char)0xfe, (char)0xff, (char)0x00, (char)0xfe, (char)0xff, (char)0x00,
   (char)0xfe, (char)0xff, (char)0x00, (char)0xfc, (char)0x7f, (char)0x00,
   (char)0xf8, (char)0x3f, (char)0x00, (char)0xf0, (char)0x1f, (char)0x00,
   (char)0xe0, (char)0x0f, (char)0x00, (char)0xe0, (char)0x0f, (char)0x00,
   (char)0xe0, (char)0x0f, (char)0x00, (char)0xe0, (char)0x0f, (char)0x00,
   (char)0xe0, (char)0x0f, (char)0x00, (char)0xf0, (char)0x1f, (char)0x00,
   (char)0xf8, (char)0x3f, (char)0x00, (char)0xfc, (char)0x7f, (char)0x00,
   (char)0xfe, (char)0xff, (char)0x00, (char)0xfe, (char)0xff, (char)0x00,
   (char)0xfe, (char)0xff, (char)0x00, (char)0xff, (char)0xff, (char)0x01,
...
readScene(FileInfo &files)
{
    ...
    if (files.count == 0) // stdin is represented as "-" also if args
        return 0;         // are empty (parse_args will take care of it)
    ...
    if (files.count==1 && strcmp(files.names[0], "-")==0) {
        ...
        if (!OPEN_INPUT_FILE(&in, files.names[0], FALSE, NULL))
            return 0;

        root = SoDB::readAll(&in);
        if (root == NULL)
            FILE_READ_ERROR(files.names[0], NULL, FALSE);
        else
            root->ref();

        CLOSE_INPUT_FILE(&in, files.names[0]);
    }
        ...
        for (int i=0; i<files.count; i++) {
            ...
            if (!OPEN_INPUT_FILE(&in, files.names[i], FALSE, NULL)) {
                root->unref();
                return 0;
            }
            ...
            if (inputRoot == NULL) {
                root->unref();
                FILE_READ_ERROR(files.names[i], NULL, FALSE);
                return 0;
            }
            root->addChild(inputRoot);
            
            CLOSE_INPUT_FILE(&in, files.names[i]);
        }
    }
...
optimizationChanged(FileInfo &files, SbBool maintainCamera)
{
...
#if 0 // Not ported yet. Actually, this functionality is not accessible,
      // because the menu is not ported yet. PCJohn 2005-03-15
    if (ivfix) {
        SbBool err = FALSE;
        char *tempFile = NULL;
        setBusyCursor(TRUE);
    ...
    // This code has been modified, because of using superscene (files.super)
    // as a workaround of overlays. PCJohn 2005-03-15
    ((SoGroup*)files.super->getChild(0))->removeAllChildren();
    if (files.optimized) {
        // optimized AND/OR ivfixed
        ((SoGroup*)files.super->getChild(0))->addChild(files.optimized);
        if (!currentCamera)
          viewer->viewAll();
    } else {
        ((SoGroup*)files.super->getChild(0))->addChild(files.original);
        if (!currentCamera)
          viewer->viewAll();
    }
...
#ifdef USE_WIN32
static void
initPerfMeter(SoWinViewer *vwr)
#else
static void
initPerfMeter(SoQtViewer *vwr)
#endif
{
    //
    // Create the overlay scene
    //
    SoAnnotation *perfMeter = new SoAnnotation;
    ...
    overlayCamera = new SoOrthographicCamera;
    perfMeter->addChild(overlayCamera);
...
#if 0 // This code is no longer used because overlays are not supported on
      // all platforms. Therefore, superscene workaround is used. PCJohn 2005-03-15
    SbColor col(1, 1, 1);
    vwr->setOverlayColorMap(1, 1, &col);
    vwr->setOverlaySceneGraph(perfMeter);
#endif
    ((SoGroup*)files.super->getChild(1))->removeAllChildren();
    ((SoGroup*)files.super->getChild(1))->addChild(perfMeter);
...
{
#if 0 // Overlays is not used (see comments in previous function).
      // So, viewport is no longer needed. Instead, the correct
      // camera projection is set up. PCJohn 2005-03-15
    if (action->isOfType(SoGLRenderAction::getClassTypeId()))
        glViewport(0, 0, 40, 40);
#endif
    if (action->isOfType(SoGLRenderAction::getClassTypeId())) {
        const SbViewportRegion &vr = ((SoGLRenderAction*)action)->getViewportRegion();
        float pointSize = overlayCamera->height.getValue() / vr.getWindowSize()[1];
        float posX = pointSize * vr.getWindowSize()[0] / 2.f - pointSize * 20.f;
        float posY = overlayCamera->height.getValue() * 0.5f - pointSize * 20.f;
        overlayCamera->position.setValue(posX, posY, 1.f);
    }
}
...
#ifdef USE_WIN32
    if (((SoWinExaminerViewer *)viewer)->isAnimating()) {
#else
    if (((SoQtExaminerViewer *)viewer)->isAnimating()) {
#endif
        if (wasAnimating) {
...
#ifdef USE_WIN32
static void
viewStartCB(void *data, SoWinViewer *)
#else
static void
viewStartCB(void *data, SoQtViewer *)
#endif
{
    char str[150];
    SoText2 *perfText = (SoText2 *) data;
...
{
#if 0 // FIXME: Missing portable cursor functionality. PCJohn 2005-03-15
...
showAboutDialog()
{
#if 0 // Should be this ported?. PCJohn 2005-03-15
...
getNewScene()
{
#if 0 // FIXME: This callback should be implemented when menu
      // functionality is ported. PCJohn 2005-03-15
...
#if 0 // FIXME: Menu event handler should be implemented when menu
      // functionality is ported. PCJohn 2005-03-15
static void
...
//
#if 0 // FIXME: X-window creation code. This should be ported by using
      // Qt and WinAPI code. PCJohn 2005-03-15
static Widget
...
#ifdef USE_WIN32
static SoWinViewer *
buildAndLayoutViewer(HWND parent, HWND menuWidget, SbBool walk)
#else
static SoQtViewer *
buildAndLayoutViewer(QWidget *parent, QWidget *menuWidget, SbBool walk)
#endif
{
#if 0 // FIXME: X-window creation code. This should be ported by using
      // Qt and WinAPI code. PCJohn 2005-03-15
...
main(int argc, char **argv)
{
    ...
    // Create super scene
    // Appended by PCJohn because of overlays workaround.
    files.super = new SoGroup;
    files.super->ref();
    files.super->addChild(new SoSeparator);
    files.super->addChild(new SoSeparator);
    viewer->setSceneGraph(files.super);
