...
IfBuilder::build(const IfShapeList &shapeList, SbBool _doStrips, SbBool _doVP,
		 SbBool _doAnyNormals, SbBool _doAnyTexCoords, SbBool useSoTransform)
{
    doStrips       = _doStrips;
    doVP           = _doVP;
    ...
    buildRoots(1, shape, useSoTransform);
	...
	if (shape->differenceLevel > 0)
	    buildRoots(shape->differenceLevel, shape, useSoTransform);

	// Always add the appropriate nodes for Level 5
	else
	    shape->addNodesForLevel(roots[5], 5, useSoTransform);
...
void
IfBuilder::buildRoots(int startLevel, IfShape *shape, SbBool useSoTransform)
{
	...
	shape->addNodesForLevel(roots[level], level, useSoTransform);
    }
