...
print_usage()
{
    (void)fprintf(stderr, "Usage: %s [-bch] [-o outfile] [infile]\n",
                  progname);
    ...
    (void)fprintf(stderr, "The utility converts vertex shapes in input to use\n");
    (void)fprintf(stderr, "Vertex Property nodes. Instanced nodes are not converted\n");
    (void)fprintf(stderr, "unless -c is specified and copies of nodes are made.\n"); 
...
main(int argc, char **argv)
{
    ...
    OPEN_INPUT_FILE(&in, argv[optind], FALSE, print_usage);
      
    root = SoDB::readAll(&in);
    if (root == NULL)  FILE_READ_ERROR(argv[optind]);
    root->ref();

    CLOSE_INPUT_FILE(&in, argv[optind-1]);
    ...
    OPEN_OUTPUT_FILE(&out, outputfile, FALSE, print_usage);
